...
//   builder.Connect(desired_state_from_position->get_output_port(),
  ...
  std::unique_ptr<systems::Context<double>> diagram_context = sys->CreateDefaultContext();
  sys->SetDefaultContext(diagram_context.get());

  systems::Context<double>& plant_context = sys->GetMutableSubsystemContext(plant, diagram_context.get());
  Eigen::VectorXd q = plant.GetPositions(plant_context, iiwa_instance);

// Holding the handle horizontal
// //   q[0] = 0*3.14/180;
//   q[1] = 50*3.14/180;//39.75*3.14/180;
//   // q[2] = 0*3.14/180;
//   q[3] = -80*3.14/180;//-69*3.14/180;
//   // q[4] = 0*3.14/180;
//   q[5] = -38*3.14/180;//70*3.14/180;//58*3.14/180;
// //   q[5] = -46*3.14/180;//58*3.14/180;
//   q[6] = -90*3.14/180;

// Holding the handle vertical
//   mq_robot(0) = 0*3.14/180;
  ...
  const math::RigidTransform<double> current_link_pose = plant.EvalBodyPoseInWorld(plant_context, plant.GetBodyByName("iiwa_link_ee"));
  const math::RollPitchYaw<double> rpy(current_link_pose.rotation());
  drake::log()->info("iiwa_link_ee at: {} {}", current_link_pose.translation().transpose(), rpy.vector().transpose());

  Simulator<double> simulator(*sys, std::move(diagram_context));
